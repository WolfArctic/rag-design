...
from selfrag_llm import MyLLM

# 定义一个包含两个LLM模型名称的列表，供用户选择
LLM_MODELS = ["None", "gpt-3.5-turbo", "gpt-4o", "selfrag/selfrag_llama2_7b", "llama-3-chinese-8b-instruct-v3"]
EMBEDDING_MODELS = ["None", "facebook/contriever-msmarco", "BAAI/bge-large-en-v1.5"]
RERANK_MODELS = ["None", "facebook/contriever-msmarco", "BAAI/bge-large-en-v1.5"]
RETRIVER_MODELS = ["None", "facebook/contriever-msmarco", "BAAI/bge-large-en-v1.5"]
...
def llm_reply(collection, chat_history, model, retriever_model, max_length=256, temperature=1):
    question = chat_history[-1][0]
    print('question:',question)
    # 使用流式生成方法从模型中获取回复
    response = llm.stream(question, collection, llm_model=model, retriever_model_name_or_path=retriever_model, max_length=max_length, temperature=temperature)
    ...
    # 逐块处理模型生成的回复
    top_response = list()
    top_response.append(response[0])
    for chunk in  top_response:
        print('chunk:',chunk)
...
def match_model(model):
    if model == LLM_MODELS[3]:
        return EMBEDDING_MODELS[1], RERANK_MODELS[1], RETRIVER_MODELS[1]
    elif model == LLM_MODELS[4]:
        return EMBEDDING_MODELS[2], RERANK_MODELS[2], RETRIVER_MODELS[2]
    else:
        return EMBEDDING_MODELS[0], RERANK_MODELS[0], RETRIVER_MODELS[0]
...
with gr.Blocks(fill_height=True) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            with gr.Row():  
                ...
                rerank_model = gr.Dropdown(
                    choices=RERANK_MODELS,
                    value=RERANK_MODELS[1],    
                    label="Rerank Model",
                    interactive=False,
                    scale=1
                )
                retriever_model = gr.Dropdown(
                    choices=RETRIVER_MODELS,
                    value=RETRIVER_MODELS[1],    
                    label="Retriver Model",
                    interactive=False,
                    scale=1
                )
                # 当mode发生变化时，后面的三个模型也相应的自动根据model的变化而变化
                model.change(fn=match_model, inputs=model, outputs=[embedding_model, rerank_model, retriever_model])
        ...
        with gr.Column(scale=1, show_progress=True) as column_config:
            ...
            # 可以点击按钮重新把知识库向量化
            load_knowledge = gr.Button("重新加载知识库")
            # 创建一个文件上传控件，支持多种文件类型
    ...
    user_input.submit(fn=submit,
        ...
        fn=llm_reply,
        inputs=[collection, chatbot, model, retriever_model, max_length, temperature],
                # collection: 用户选择的知识库。
                # chatbot: 当前的聊天记录（已经包含用户的新查询）。
                # model: 用户选择的 LLM 模型。
                # retriever_model: 用户选择的 retriever 模型。
    ...
    # 绑定重新加载知识库按钮的点击事件，调用upload_knowledge函数，并更新知识库下拉菜单
    load_knowledge.click(fn=llm.upload_knowledge, inputs=None, outputs=[collection])
